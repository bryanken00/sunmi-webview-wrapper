// Sunmi built-in printer AIDL, copied verbatim from the official interface.
//
// TRUNCATED ON PURPOSE after printOriginalText(). AIDL assigns binder
// transaction codes by DECLARATION ORDER, so every method kept below keeps
// the exact same code it has in the full interface and stays wire-compatible
// with the service on the device. The methods that follow it in the original
// (commitPrint, tax, ...) drag in the TransBean and ITax types, which we would
// otherwise have to vendor for no reason -- we do not call them.
//
// NEVER reorder, insert, or delete a method above. Doing so silently shifts
// transaction codes and calls land on the wrong method.
